...
class YoloPoseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pad_img = cv2.copyMakeBorder(img, 0, dh, 0, dw, cv2.BORDER_CONSTANT, value=(114, 114, 114))
        
        img_tensor = torch.from_numpy(pad_img).permute(2, 0, 1).float() / 255.0
        ...
        if os.path.exists(label_path):
            with open(label_path, 'r') as f:
                for line in f:
                    ...
                    kpts = data[5:]
                    if len(kpts) // self.nkpts == 2:
                        for i in range(self.nkpts):
                            kpts.insert(3*i+2, 1)
                    
                    targets.append([0, cls] + box.tolist() + kpts)
        
        # labels_out = torch.zeros((len(targets[0]))) if targets else torch.zeros((0, 6 + self.nkpts*3))
        # if targets:
        labels_out = torch.tensor(targets, dtype=torch.float32)
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        """
        pred_dist: (N_pos, 4, 16) - DFL 예측 분포
        pred_bboxes: (N_pos, 4) - 디코딩된 좌표
        anchor_points: (N_pos, 2)
...
def train(ckps=None):
    ...
    model = DINOv3Pose(backbone='dinov3_convnext_base', nkpts=(4, 3), ncls=10, device=device)
    if ckps:
