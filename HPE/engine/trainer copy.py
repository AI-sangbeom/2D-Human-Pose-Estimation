...
class Trainer(object):
    ...
    def train(self, traindataloader, valdataloader, startepoch, endepoch):
        for epoch in range(startepoch, endepoch+1):
            ...
            if epoch%self.args.save_interval==0:
                ...
                path = os.path.join(self.args.saveDir, 'model_{}.pth'.format(epoch))
                torch.save(state, path)
            adjust_learning_rate(self.optimizer, epoch, self.args.dropLR, self.args.dropMag)
    ...
    def _epoch(self, dataloader, epoch, mode = 'train'):
        ...
        for batch_idx, (data, target, meta1, meta2) in enumerate(dataloader):
            ...
            if mode == 'train':
                    loss.backward()
                    if (batch_idx+1)%self.args.mini_batch_count==0:
                            self.optimizer.step()
