...
from models.nn.block import ConvNext2Block as Block
from models.nn import SpBlock, SpLayerNorm, LayerNorm
...
class SparseConvNeXtV2(nn.Module):
    ...
    def __init__(self, 
        ...
        # Stem: dense convolution (4x downsampling)
        stem = nn.Sequential(
    ...
    def forward(self, x, mask):
        """
        Args:
            x: Input tensor [B, C, H, W] (e.g., [2, 3, 224, 224])
            mask: Binary mask [B, N] where N = num_patches
                  예: H=224, W=224, patch_size=4 -> N = (224/4)^2 = 3136
        ...
        # Stem을 통과하기 전에 mask를 입력 해상도로 upsample
        # mask: [B, N] -> [B, p, p] -> [B, H, W]
        # stem의 stride=4를 고려하여 mask를 4배 upsample
        mask = self.upsample_mask(mask, 4)  # [B, H, W]
        mask = mask.unsqueeze(1).type_as(x)  # [B, 1, H, W]
        
        # 입력에 mask 적용 (stem 전에)
        x = x * (1. - mask)  # Masked positions를 0으로
        
        # Patch embedding (dense) - 4x downsampling
        x = self.downsample_layers[0](x)  # [B, dims[0], H/4, W/4]
